...
对 WorkBuddy 失败请求做原始抓包后，Mantle 返回的标准错误信封为：

```json
{
  "error": {
    "code": "validation_error",
    "message": "Duplicate item found with id rs_afb67a0949735055914dc5c0640b4388. Remove duplicate items from your input and try again.",
    "param": null,
...
该请求包含 56 条 Chat messages 和 25 个工具结果。网关从 `rsc_v1` capsule 解出 15
个 reasoning 回放项，但只有 14 个唯一 ID；上述 reasoning item 被两个工具调用 capsule
各携带一次，且载荷完全相同。Chat 客户端保留两个工具调用是正确行为，但适配器把两个
capsule 独立展开后，在发往 Responses API 的 `input` 中制造了重复 item ID。

修复在 Chat 历史转换完成后、序列化 Mantle 请求前执行稳定去重：

- 同一 reasoning ID 且完整载荷相同：只保留第一次出现，其他消息和工具项的顺序不变；
- 同一 reasoning ID 但载荷不同：本地返回 400，不静默选择任一 signed reasoning；
- function call 和 function output 不去重，各自仍按原始 Chat 历史完整回放。

这项处理不依赖模型名，也不需要 Redis 或数据库；它只修正
`chat_backend = "responses"` 无状态协议适配产生的重复 reasoning item。
...
```json
{
  "error": {
    "code": "validation_error",
    "message": "prompt tokens (280007) exceed customer model maximum (278528) ...",
    "param": null,
    "type": "invalid_request_error"
  }
}
...
网关会解析不超过 64 KiB 的标准 OpenAI 上游错误信封，并原样返回给当前客户端。错误
正文不会进入 `AppError` 的 `Display`，因此 CloudWatch 仍只记录状态码，不记录提示词、
响应正文或上游错误正文。非 JSON、超限错误正文继续使用通用状态映射。
...
- WorkBuddy 连续工具会话在第四次续轮加入 assistant 文本 parts 后仍可继续，不再触发
  Mantle 400；
- 24 个高推理流式工具轮（含 8 次重复 `call_0`）、最终文本以及追加用户续轮全部完成，
  证明轮数、重复 Responses `call_id` 和“最终回答后续问”本身不会触发 Mantle 400；
- 真实 WorkBuddy 失败请求精确复现重复 reasoning item ID；修复后将同一份 695630
